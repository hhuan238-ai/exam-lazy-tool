...
頁面模式也有「公式計算機」，EOQ、再訂購點、Newsvendor 可以直接輸入數值計算，不需要貼資料表。
...
REORDER_POINT_SS([D], [L], [SS], [periods])
...
- `REORDER_POINT_SS([D], [L], [SS], [periods])`：若題目給年需求與每年期數，例如 `REORDER_POINT_SS(10000, 4, 55, 52)`，會先算 `10000 / 52 * 4` 再加 `SS`。
